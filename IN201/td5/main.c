...
void yield( void ) {
    ...
    // mprotect( stack1, STACK_SIZE, PROT_READ | PROT_WRITE );
...
void scheduler_rtn( void ) {
    ...
    char *list_stacks[] = { stack2, stack3, stack4 };

    mprotect( stack2, STACK_SIZE, PROT_NONE );
    mprotect( stack3, STACK_SIZE, PROT_NONE );
    mprotect( stack4, STACK_SIZE, PROT_NONE );
    ...
    while ( 1 ) {
        ...
        case STATUS_READY:
            ...
            current_thread = fb;
            mprotect( list_stacks[current], STACK_SIZE, PROT_READ | PROT_WRITE );
            // mprotect( stack1, STACK_SIZE, PROT_NONE );
            switch_coroutine( &scheduler, ( fb->cr ) );
            mprotect( list_stacks[current], STACK_SIZE, PROT_NONE );
            break;
...
void memory_error() {
    //printf( "Task %d caused memory error\n", current );
    kill( 0, SIGKILL );
}
