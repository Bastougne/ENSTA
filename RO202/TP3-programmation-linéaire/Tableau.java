...
public class Tableau {
    ...
    public boolean pivot() {
        ...
        double coeff;
        for ( int iPivot = 0; iPivot < m; iPivot++ ) {
            coeff = 1.0 / A[iPivot][basis[iPivot]]; // normalisation de A[iPivot][basis[iPivot]] dans A[iPivot][]
            for ( int j = 0; j < n; j++ ) {
                A[iPivot][j] *= coeff;
            }
            b[iPivot] *= coeff;

            for ( int i = iPivot + 1; i < m; i++ ) { // annulation de A[i][basis[iPivot]] par CL dans A[i][]
                coeff = A[i][basis[iPivot]];
                for ( int j = 0; j < n; j++ ) {
                    A[i][j] -= coeff * A[iPivot][j];
                }
                b[i] -= coeff * b[iPivot];
            }

            coeff = c[basis[iPivot]]; // mise à jour des contraintes et de bestObjective
            for ( int j = 0; j < n; j++ ) {
                c[j] -= coeff * A[iPivot][j];
            }
            bestObjective -= coeff * b[iPivot];
        ...
        int e = -1;
        double meilleurDelta = 0; // Cette intialisation implique que xEntrant ne sera modifié que si au moins un cout réduit est du "bon" signe
        for ( int i = 0; i < n; i++ ) {
            if ( isMinimization ) {
                if ( c[i] < meilleurDelta - epsilon ) {
                    e = i;
                    meilleurDelta = c[i];
                }
            } else {
        ...
        if ( e != -1 )
            System.out.println( "Variable in : x" + ( e + 1 ) );

        if ( e == -1 ) // pas d'amélioration possible
            return false;
        ...
        for ( int i = 0; i < m; i++ ) {
            if ( b[i] / A[i][e] > meilleurRatio + epsilon ) {
                s = i;
                meilleurRatio = b[i] / A[i][e];
            }
        }

        System.out.println( "Variable out : x" + ( s + 1 ) );
