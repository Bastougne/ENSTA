public class Tableau {
    ...
    public boolean pivot() {
        ...
        for ( int i = 0; i < n; i++ )
            if ( ( !isMinimization && c[i] > 1E-6 && c[i] > bestReducedCost ) || ( isMinimization && c[i] < -1E-6 && c[i] < bestReducedCost ) ) {
                bestEnteringVar = i;
                bestReducedCost = c[i];
            }
        ...
        if ( bestEnteringVar != -1 ) {
            ...
            for ( int constraintId = 0; constraintId < m; constraintId++ ) {
                 ...
                 */
                if ( ratio > 1E-6 && ratio < bestRatio || Math.abs( ratio ) < 1E-6 && A[constraintId][bestEnteringVar] > -1E-6 ) {
                    bestRatio = ratio;
