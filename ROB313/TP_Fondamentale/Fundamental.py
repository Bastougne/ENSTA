...
[success,H1,H2] = cv2.stereoRectifyUncalibrated(pts1,pts2,FRansac,imgL.shape[0:2])
imgLtrans = cv2.warpPerspective(img1,H1,imgL.shape[0:2])
imgRtrans = cv2.warpPerspective(img2,H2,imgR.shape[0:2])
print('Rectification des images : ' + str(success))

plt.figure(figsize=(15, 5))
plt.subplot(121),plt.imshow(imgLtrans)
plt.title('Rectification (gauche)')
plt.subplot(122),plt.imshow(imgRtrans)
plt.title('Rectification (droite)')
plt.show()
