...
def main():
    ...
    n = 1
    iter_moy, len_moy, n_ok = 0, 0, 0
    for i in range( n ):
        rrt_star = RrtStar(environment, x_start, x_goal, 3, 0.10, 20, 1500)
        path, nb_iter = rrt_star.planning()

        if path:
            iter_moy = iter_moy + nb_iter
            len_moy = len_moy + get_path_length( path )
            n_ok = n_ok + 1
    ...
    iter_moy, len_moy = iter_moy / n_ok, len_moy / n_ok
    print( iter_moy )
    print( len_moy )
