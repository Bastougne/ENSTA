...
bool find_word_dicho (char **words, char *word, int left, int right) {
    ...
    int diff = strcmp( word, words[pivot] );
    if ( left > right ) {
        return( false );
    }
    if ( diff == 0 ) {
        ...
        return( find_word_dicho( words, word, left, pivot - 1 ) );
    } else {
        return( find_word_dicho( words, word, pivot + 1, right ) );
    }
}
...
int main( int argc, char* argv[] ) {
    ...
    char buffer[30];
    if ( argc != 2 ) { // On vérifie que les entrées sont bien valides
    ...
    while ( gets( buffer ) ) {
        if ( find_word_dicho( words, buffer, 0, num_words ) ) {
            printf( "Trouvé\n" );
        }
        else {
            printf( "Non trouvé\n" );
