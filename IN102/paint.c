...
void destroy_pixels( unsigned char *pixels ) { free( pixels ); }

void color_pixel( int x, int y, unsigned char r, unsigned char g,
...
void get_pixel_color( int x, int y, unsigned char *r, unsigned char *g,
                      unsigned char *b, int width, int height,
                      unsigned char *pixels ) {
    // don't paint outside image
    if ( x < 0 ) {
...
int save_BMP( char *filename, int width, int height, unsigned char *pixels ) {
    ...
    static unsigned char header[54] = {
        66, 77, 0, 0, 0, 0, 0, 0, 0, 0, 54, 0, 0, 0, 40,
    ...
    *heightEntry = height;
    ...
    for ( int row = 0; row < height; row++ ) {
        // Note: BMP rows are written from the bottom up (unless height is
        // negative)
        write( fd, pixels + 3 * ( height - 1 - row ) * width,
               pixelBytesPerRow );
