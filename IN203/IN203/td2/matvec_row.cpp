...
Matrix::Matrix( int dim, int rank, int numtasks ) : m_nrows( dim / numtasks ), m_ncols( dim ), m_arr_coefs( dim * dim ) {
    int debut = rank * ( dim / numtasks );
    int fin = ( rank + 1 ) * ( dim / numtasks );
    for ( int i = debut; i < fin; ++i ) { // lignes
        for ( int j = 0; j < dim; ++j ) { // colonnes
            ( *this )( i - debut, j ) = ( i + j ) % dim;
        }
    }
}
...
int main( int argc, char *argv[] ) { // Parallélisation par colonnes                                            (   )
    ...
    std::vector<double> v = A * u; // les v_process_rank correspondent à des sous-vecteurs de v
    std::vector<double> v2( N );
    MPI_Allgather( v.data(), v.size(), MPI_DOUBLE, v2.data(), v.size(), MPI_DOUBLE, globComm );
