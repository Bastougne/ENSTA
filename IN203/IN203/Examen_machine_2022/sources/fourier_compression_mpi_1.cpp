...
std::complex<double> *discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const *pixels,
    ...
    for ( std::uint32_t k1 = 0; k1 < ni; ++k1 ) {
        for ( std::uint32_t k2 = debut_y; k2 < fin_y; ++k2 ) {
...
int main( int nargs, char *argv[] ) {
    ...
    MPI_Gather( fourier + height * ( width / numtasks ) * rank, height * ( width / numtasks ), MPI_COMPLEX, fourier,
                height * ( width / numtasks ), MPI_COMPLEX, 0, globComm );
