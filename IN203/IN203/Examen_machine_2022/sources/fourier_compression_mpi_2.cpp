...
std::complex<double> *discretTransformFourier( std::uint32_t width, std::uint32_t height, unsigned char const *pixels,
                                               unsigned int debut_y, unsigned int fin_y ) {
    ...
    for ( std::uint32_t k1 = 0; k1 < ni; ++k1 ) {
        for ( std::uint32_t k2 = 0; k2 < nj; ++k2 ) {
            for ( std::uint32_t n2 = debut_y; n2 < fin_y; ++n2 ) {
                std::complex<double> exp2( std::cos( -2 * pi * n2 * k2 / height ),
                                           std::sin( -2 * pi * n2 * k2 / height ) );
...
sparseSpectralComposition compressSpectralComposition( std::uint32_t width, std::uint32_t height,
                                                       const std::complex<double> *plainCoefs,
                                                       double tauxCompression ) {
    std::uint32_t nb_coefs = std::uint32_t( tauxCompression * width * height );
    struct node {
        node( std::uint32_t t_i, std::uint32_t t_j, double val ) : i( t_i ), j( t_j ), module( val ) {}

        bool operator<( node const &v ) const { return this->module < v.module; }

        bool operator>( node const &v ) const { return this->module > v.module; }

        bool operator<( double v ) const { return this->module < v; }
        ...
        for ( std::uint32_t n2 = 0; n2 < ni; ++n2 ) {
            ...
            for ( std::uint32_t k1 = 0; k1 < ni; ++k1 ) {
                ...
                for ( std::uint32_t k2 = sparse.begin_rows[k1]; k2 < sparse.begin_rows[k1 + 1]; ++k2 ) {
                    std::complex<double> exp2( std::cos( +2 * pi * n2 * sparse.ind_cols[k2] / nj ),
                                               std::sin( +2 * pi * n2 * sparse.ind_cols[k2] / nj ) );
...
int main( int nargs, char *argv[] ) {
    ...
    MPI_Init( &nargs, &argv ); // Initialisation de MPI
    ...
    if ( rank == 0 )
        std::cout << "Caracteristique de l'image : " << width << "x" << height << " => " << width * height << " pixels."
                  << std::endl
                  << std::flush;

    auto debut = std::chrono::system_clock::now();

    unsigned int debut_y = ( rank * width ) / numtasks;
    unsigned int fin_y = ( ( rank + 1 ) * width ) / numtasks;

    std::complex<double> *fourier = discretTransformFourier( width, height, image, debut_y, fin_y );
    std::complex<double> *fourier_tot = new std::complex<double>[height * width];
    MPI_Allreduce( fourier, fourier_tot, height * width, MPI_COMPLEX, MPI_SUM, globComm );

    auto fin = std::chrono::system_clock::now();
    double duree = (double)std::chrono::duration_cast<std::chrono::milliseconds>( fin - debut ).count();
    debut = fin;
    std::cout << "durée encodage DFT : " << duree << " ms\n";
    if ( rank == 0 ) {
        auto sparse = compressSpectralComposition( width, height, fourier_tot, taux );

        fin = std::chrono::system_clock::now();
        duree = (double)std::chrono::duration_cast<std::chrono::milliseconds>( fin - debut ).count();
        debut = fin;
        std::cout << "durée sélection coefficients : " << duree << " ms\n";
        ...
        duree = (double)std::chrono::duration_cast<std::chrono::milliseconds>( fin - debut ).count();
        debut = fin;
        std::cout << "durée reconstitution image compressée : " << duree << " ms\n";
