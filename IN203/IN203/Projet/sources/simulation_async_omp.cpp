...
#pragma omp parallel for schedule(static) // plus rapide en statique ici
...
void simulation( bool affiche, MPI_Comm globComm, int rank ) {
    ...
    for ( std::size_t i = 0; i < contexte.taux_population; ++i ) { // on ne parallélise pas ici afin de conserver la valeur de graine_aleatoire
