...
epidemie::Individu::Individu( unsigned int graine_aleatoire, int esperance_de_vie, bool estNe, int deplacement_max )
    : m_age{}, m_esperance_de_vie{ esperance_de_vie }, m_position{}, m_moteur_stochastique{ graine_aleatoire },
      m_generateur_quantite_deplacement( 1, deplacement_max ), m_generateur_deplacement( 1, 4 ),
      m_generateur_maladie( 0., 1. ), m_generateur_age( 1, esperance_de_vie ) {
    if ( estNe ) {
        m_age = 1;
    } else
...
void epidemie::Individu::estContamine( AgentPathogene &agent ) {
    if ( m_agent_pathogene.sensibilite == Sensibilite::Sensible ) {
        ...
        m_agent_pathogene.temps_contagieux = m_agent_pathogene.temps_asymptomatique +
                                             m_agent_pathogene.temps_symptomatique + agent.periode_contagieux.second;
        m_agent_pathogene.sensibilite = Sensibilite::Contagieux;
    }
}
// --------------------------------------------------------------------------------------------------------------------
bool epidemie::Individu::testContaminationGrippe( Grille const &grille, Interactions const &t_interactions,
    ...
    double probabilite_attraper_grippe = t_interactions.tauxTransmissionGrippe(
        grippe.tauxTransmission(), false, this->m_agent_pathogene.sensibilite == Sensibilite::Infecte,
    ...
    probabilite_attraper_grippe = t_interactions.tauxTransmissionGrippe(
        grippe.tauxTransmission(), true, this->m_agent_pathogene.sensibilite == Sensibilite::Infecte,
        this->m_agent_pathogene.sensibilite == Sensibilite::Immunise );
