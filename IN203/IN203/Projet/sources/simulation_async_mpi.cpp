...
void simulation( bool affiche, MPI_Comm globComm, int rank, int numtasks ) {
    ...
    int taile_sous_pop = contexte.taux_population / ( numtasks - 1 );
    ...
    while ( !quitting ) {
        ...
        if ( rank != 0 ) { // ces processus s'occupent de la simulation
            ...
            for ( int ipersonne = ( rank - 1 ) * taile_sous_pop; ipersonne < rank * taile_sous_pop; ipersonne++ ) {
                if ( population[ipersonne].testContaminationGrippe( grille, contexte.interactions, grippe, agent ) ) {
                    compteur_grippe++;
                    population[ipersonne].estContamine( grippe );
                }
                if ( population[ipersonne].testContaminationAgent( grille, agent ) ) {
                    compteur_agent++;
                    population[ipersonne].estContamine( agent );
                }
                // On verifie si il n'y a pas de personne qui veillissent de veillesse et on genere une nouvelle
                // personne si c'est le cas.
                if ( population[ipersonne].doitMourir() ) {
                    mouru++;
                    unsigned nouvelle_graine =
                        jours_ecoules + population[ipersonne].position().x * population[ipersonne].position().y;
                    population[ipersonne] =
                        epidemie::Individu( nouvelle_graine, contexte.esperance_de_vie, contexte.deplacement_maximal );
                    population[ipersonne].setPosition( largeur_grille, hauteur_grille );
                }
                population[ipersonne].veillirDUnJour();
                population[ipersonne].seDeplace( grille );
            }
...
int main( int argc, char *argv[] ) {
    ...
    sdl2::init( argc, argv );
    { simulation( affiche, globComm, rank, numtasks ); }
    sdl2::finalize();
