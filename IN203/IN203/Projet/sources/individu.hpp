...
class Individu {
  public:
    enum class Sensibilite { Sensible, Contagieux, Infecte, Immunise };

    Individu( unsigned int graine_aleatoire, int esperance_de_vie, bool estNe = false, int deplacement_max = 10 );

    Sensibilite getEtatGrippal() const { return m_grippe.sensibilite; }

    Sensibilite getEtatAgentPathogene() const { return m_agent_pathogene.sensibilite; }

    bool aGrippeContagieuse() const { return m_grippe.temps_contagieux > 0; }

    bool aAgentPathogeneContagieux() const { return m_agent_pathogene.temps_contagieux > 0; }

    void veillirDUnJour();

    bool doitMourir() const { return m_age >= m_esperance_de_vie; }
    ...
    void devientImmuniseGrippe() { m_grippe.sensibilite = Sensibilite::Immunise; }

    void redevientSensibleGrippe() { m_grippe.sensibilite = Sensibilite::Sensible; }

    bool testContaminationGrippe( Grille const &grille, Interactions const &t_interactions, Grippe &grippe,
