...
void afficher_titre( sdl2::window &fenêtre, const std::string_view &titre ) {
    ...
    sdl2::texte bannière( titre.data(), fonte_titre, fenêtre, w, 64, { 0xFF, 0xFF, 0x00, 0xFF },
                          { 0x00, 0x00, 0xFF, 0xFF }, sdl2::texte::middle_justify, 0, 11 );
...
void affiche_document( sdl2::window &fenêtre, const std::vector<std::string> &doc, int première_ligne = 0 ) {
    ...
    while ( ( posy < h ) && ( index < doc.size() ) ) {
        ...
        sdl2::texte texte( sub_string.data(), *pt_fonte, fenêtre, w, encre, { 0x00, 0x00, 0x00, 0x00 },
                           sdl2::texte::left_justify, 0 );
