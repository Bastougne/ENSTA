...
class couleur {
  public:
    ...
    */
    couleur( const char *code_couleur, bool pour_encre = true ) : couleur_ansi( code_couleur ) {
        if ( not pour_encre )
            this->couleur_ansi[2] = '4';
    }

    couleur( int index_palette256, bool pour_encre = true ) : couleur_ansi( "\u001b[38;5;" ) {
        this->couleur_ansi += std::to_string( index_palette256 ) + "m";
        if ( not pour_encre )
            this->couleur_ansi[2] = '4';
    }

    couleur( std::uint8_t rouge, std::uint8_t vert, std::uint8_t bleu, bool pour_encre = true )
        : couleur_ansi( "\u001b[38;2;" ) {
        couleur_ansi += std::to_string( rouge ) + ";" + std::to_string( vert ) + ";" + std::to_string( bleu ) + "m";
        if ( not pour_encre )
            this->couleur_ansi[2] = '4';
    }

    void convertir_pour_encre() { this->couleur_ansi[2] = '3'; }

    void convertir_pour_papier() { this->couleur_ansi[2] = '4'; }

    const std::string &code_ansi() const { return this->couleur_ansi; }
...
inline std::ostream &operator<<( std::ostream &out, const console::couleur &coul ) {
    out << coul.code_ansi();
    return out;
}
