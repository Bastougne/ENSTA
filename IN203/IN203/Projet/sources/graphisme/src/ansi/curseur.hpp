...
class curseur {
  public:
    ...
    curseur( const position &pos ) : ansi_escape( "\u001b[" ) {
        this->ansi_escape += std::to_string( pos[0] ) + ";" + std::to_string( pos[1] ) + "H";
    }

    curseur( const char *direction, int nombre_pas ) : ansi_escape( "\u001b[" ) {
    ...
    curseur( const char *commande ) : ansi_escape( commande ) {}

    const std::string &code_ansi() const { return this->ansi_escape; }
...
inline std::ostream &operator<<( std::ostream &out, const console::curseur &cur ) {
    out << cur.code_ansi();
    return out;
}
