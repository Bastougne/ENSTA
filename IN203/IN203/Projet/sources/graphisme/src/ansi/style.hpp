...
class style {
  public:
    ...
    style( const char *commande ) : ansi_escape( commande ) {}

    style( const std::initializer_list<const char *> &commandes ) : ansi_escape() {
        for ( auto commande : commandes )
            this->ansi_escape += commande;
    }
    style( int commande, const std::string &phrase ) : ansi_escape() {
        if ( commande == 1 ) {
            this->ansi_escape += "\u001b[s\u001b#3" + phrase + "\u001b[u\u001b[B\u001b#4" + phrase;
        } else if ( commande == 2 ) {
    ...
    const std::string &ansi_code() const { return this->ansi_escape; }
...
inline std::ostream &operator<<( std::ostream &out, const console::style &s ) {
    out << s.ansi_code();
    return out;
}
