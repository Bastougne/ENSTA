...
class event_mouse_motion : public event {
  public:
    event_mouse_motion( SDL_MouseMotionEvent *event )
        : position_x( event->x ), position_y( event->y ), relative_x( event->xrel ), relative_y( event->yrel ) {}

    virtual std::int32_t kind_of_event() const override { return event::mouse_motion; }

    std::array<std::int32_t, 2> get_relative_movement() const { return { this->relative_x, this->relative_y }; }

    std::array<std::int32_t, 2> position() const { return { this->position_x, this->position_y }; }
...
class event_mouse_button : public event {
  public:
    ...
    event_mouse_button( SDL_MouseButtonEvent *event )
        : _id_button( event->button ), _state_of_button( event->state ), _number_of_clicks( event->clicks ),
          _position_x( event->x ), _position_y( event->y ) {}

    virtual std::int32_t kind_of_event() const override { return event::mouse_button; }
    ...
    std::array<std::int32_t, 2> position() const { return { this->_position_x, this->_position_y }; }
...
class event_keyboard : public event {
  public:
    ...
    event_keyboard( SDL_KeyboardEvent *event )
        : _state( event->state ), _code( event->keysym.sym ), _repeat( event->repeat ) {}
...
class event_touch : public event {
  public:
    event_touch( SDL_TouchFingerEvent *evt )
        : x( evt->x ), y( evt->y ), dx( evt->dx ), dy( evt->dy ), pressure( evt->pressure ), type( evt->type ) {}

    virtual std::int32_t kind_of_event() const override { return event::touch_finger; }

    std::int32_t type_of_event() const { return this->type; }

    std::array<float, 2> where() const { return { this->x, this->y }; }

    std::array<float, 2> displacement() const { return { this->dx, this->dy }; }
