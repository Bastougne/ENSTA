...
auto sdl2::formated_text::font_manager_t::load_and_add( const std::string &key, const std::string &fonte,
                                                        std::uint8_t size ) -> iterator {
...
sdl2::formated_text::formated_text( const std::array<std::int32_t, 4> &z, const std::string &path_for_font )
    : image(), font_manager(), splitted_string(), begin_line( 0 ), zone( z ), formats(), ordered_list_formats(),
      unordered_list_formats(), qformat() {
#if defined( DEBUG )
    ...
    formats[style::italic] = {
        this->font_manager["italic"], { 0xB0, 0xB0, 0xB0, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 8 };
    formats[style::bold] = { this->font_manager["bold"], { 0xFF, 0xFF, 0xFF, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 8 };
    formats[style::italic_bold] = {
        this->font_manager["bolditalic"], { 0xE0, 0xFF, 0xFF, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 8 };
    formats[style::heading_1] = {
        this->font_manager["heading1"], { 0xFF, 0xFF, 0x00, 0xFF }, { 0x00, 0x00, 0xF0, 0xFF }, 0 };
    formats[style::heading_2] = {
        this->font_manager["heading2"], { 0x00, 0xFF, 0xFF, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 2 };
    formats[style::heading_3] = {
        this->font_manager["heading3"], { 0xFF, 0xFF, 0xFF, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 4 };
    formats[style::heading_4] = {
        this->font_manager["heading4"], { 0xFF, 0xFF, 0xFF, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 6 };
    formats[style::heading_5] = {
        this->font_manager["heading5"], { 0xFF, 0xA0, 0xA0, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 8 };
    formats[style::heading_6] = {
        this->font_manager["heading6"], { 0xFF, 0x00, 0x00, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 10 };
    ...
    for ( int i = 0; i < max_list_level; ++i ) {
        this->ordered_list_formats[i] = {
            { 0xFF, 0xFF, 0xFF, 0xFF }, { 0x00, 0x00, 0x00, 0xFF }, 32 + 16 * i, numbering::latin };
...
void sdl2::formated_text::begin_at_line( window &w, int iline ) {
    ...
    SDL_FillRect( panneau, nullptr,
                  SDL_MapRGB( panneau->format, this->formats[style::texte].papier.red,
                              this->formats[style::texte].papier.green, this->formats[style::texte].papier.blue ) );
    style current_style = style::texte;
    SDL_Rect pos;
    pos.y = 0;
    for ( std::size_t iwords = iline; ( iwords < this->splitted_string.size() ) && ( pos.y < maxheight ); ++iwords ) {
#if defined( DEBUG )
        ...
        if ( this->splitted_string[iwords].size() > 0 )
            std::cerr << "Premier mot : " << this->splitted_string[iwords].front()
                      << ", dernier mot : " << this->splitted_string[iwords].back() << std::endl
                      << std::flush;
#endif
        ...
        for ( std::size_t iword = 0; iword < words.size(); ++iword ) {
            ...
            SDL_FillRect( panneau, &papier_fill,
                          SDL_MapRGB( panneau->format, this->formats[current_style].papier.red,
                                      this->formats[current_style].papier.green,
            ...
            SDL_Surface *textSurface =
                TTF_RenderUTF8_Solid( this->formats[current_style].pt_fonte->m_pt_ttf, true_word.c_str(), col );
    ...
    if ( this->pt_texture == nullptr ) {
        std::string errMsg = "Incapable de créer une texture du texte rendu : " + std::string( SDL_GetError() );
