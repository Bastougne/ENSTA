...
sdl2::rectangle::rectangle( const std::array<std::uint16_t, 2> &position, const std::array<std::uint16_t, 2> &dimension,
                            const sdl2::color &col, bool fill )
    : x( position[0] ), y( position[1] ), width( dimension[0] ), height( dimension[1] ), is_filled( fill ), ink( col ) {
}
// =================================================================================================
void sdl2::rectangle::render( window &win ) const {
    SDL_Rect rect = { this->x, this->y, this->width, this->height };
    SDL_SetRenderDrawColor( win.get_native_renderer(), this->ink.red, this->ink.green, this->ink.blue,
                            this->ink.alpha );
    if ( this->is_filled )
...
// =================================================================================================
sdl2::line::line( const std::array<std::uint16_t, 2> &beg_pos, const std::array<std::uint16_t, 2> &end_pos,
                  const color &col )
    : beg_x( beg_pos[0] ), beg_y( beg_pos[1] ), end_x( end_pos[0] ), end_y( end_pos[1] ), ink( col ) {}
// =================================================================================================
void sdl2::line::render( window &win ) const {
...
// =================================================================================================
void sdl2::polygon::render( window &win ) const {
    SDL_SetRenderDrawColor( win.get_native_renderer(), ink.red, ink.green, ink.blue, ink.alpha );
