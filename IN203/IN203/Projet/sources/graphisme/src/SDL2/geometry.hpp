...
namespace sdl2 {
struct rectangle {
    rectangle( const std::array<std::uint16_t, 2> &position, const std::array<std::uint16_t, 2> &dimension,
               const color &col, bool fill = true );
...
struct line {
    line( const std::array<std::uint16_t, 2> &beg_pos, const std::array<std::uint16_t, 2> &end_pos, const color &col );
    void render( window &win ) const;
