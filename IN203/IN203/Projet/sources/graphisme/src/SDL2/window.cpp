...
sdl2::window::window() : window( "SDL2", { -1, -1 }, { SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED }, true ) {}
// ======================================================================================
sdl2::window::window( const std::string &titre, const std::array<int, 2> &dimensions,
                      const std::array<int, 2> &position, bool affiche )
    : m_window( nullptr ), m_pt_renderer( nullptr ) {
    SDL_DisplayMode dispMode;
    SDL_GetCurrentDisplayMode( 0, &dispMode );
    // Tentative de création de la fenêtre :
    m_window = SDL_CreateWindow(
        titre.c_str(), position[0], position[1], ( dimensions[0] == -1 ? dispMode.w : dimensions[0] ),
        ( dimensions[1] == -1 ? dispMode.h : dimensions[1] ), affiche ? SDL_WINDOW_SHOWN : SDL_WINDOW_HIDDEN );
