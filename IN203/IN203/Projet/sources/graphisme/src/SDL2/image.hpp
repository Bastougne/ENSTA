...
class image {
  public:
    enum sens { none = SDL_FLIP_NONE, horizontal = SDL_FLIP_HORIZONTAL, vertical = SDL_FLIP_VERTICAL };
    image()
        : pt_texture( nullptr ), position_and_stretching(), center_rotation_x( -1 ), center_rotation_y( -1 ),
          angle( 0. ), filter_color( 0x00, 0x00, 0x00, 0x00 ), m_direction( SDL_FLIP_NONE ) {}

    image( const std::string &file, window &win, const color &transparent = { 0x00, 0x00, 0x00, 0x00 } );
    virtual ~image();
