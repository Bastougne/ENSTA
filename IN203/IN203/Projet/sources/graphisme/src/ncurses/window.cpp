...
ncurses::window::window() : pt_ncurses_window( nullptr ), pt_border_window( nullptr ) {
    this->pt_ncurses_window = stdscr;
}

ncurses::window::window( const std::array<int, 2> &dimension, const std::array<int, 2> &start, bool has_border,
                         bool enable_scrolling )
    : pt_ncurses_window( nullptr ), pt_border_window( nullptr ) {
...
ncurses::window &ncurses::window::at( int row, int col ) {
    wmove( this->pt_ncurses_window, row, col );
    return *this;
}

void ncurses::window::print( const std::string &phrase ) { wprintw( this->pt_ncurses_window, "%s", phrase.c_str() ); }

void ncurses::window::print( const ncurses::symbol &symb ) { waddch( this->pt_ncurses_window, symb.val ); }

std::array<int, 2> ncurses::window::get_cursor_position() const {
    int y, x;
