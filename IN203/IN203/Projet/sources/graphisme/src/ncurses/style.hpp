...
class style {
  public:
    ...
    style( int mode ) : value_mode( mode ) {}

    style( const std::initializer_list<int> &modes ) : value_mode( 0 ) {
        for ( const auto &mode : modes )
            value_mode |= mode;
    }

    ~style() { attroff( this->value_mode ); }
    ...
    static int number_of_color_attribut() { return COLOR_PAIRS; }

    static void define_color_attribut( int num, int encre, int fond ) {
        assert( num >= 0 );
        assert( num < COLOR_PAIRS );
        assert( encre >= 0 );
        assert( encre < COLORS );
        assert( fond >= 0 );
        assert( fond < COLORS );
    ...
    void remove_mode( int mode ) { this->value_mode &= ~mode; }

    void apply_mode( window &w ) const { wattron( w.get_native_window(), this->value_mode ); }

    void unapply_style() const { attroff( this->value_mode ); }
