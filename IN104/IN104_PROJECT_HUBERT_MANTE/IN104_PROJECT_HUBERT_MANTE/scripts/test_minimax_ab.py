...
# avec la commande python -m IN104_PROJECT_NOM1_NOM2.scripts.test_minimax_ab
import random
import argparse
import time
...
def test(root, solution):
    ...
    known_states = {}
    result = minimax_ab(root, root.maximizing, -10000, 10000, get_children, evaluate, float('inf'), known_states)
...
if __name__ == '__main__':
    ...
    parser.add_argument('version', nargs='?', default='limited_depth_ab')
    parser.add_argument('-n', '--ntests', type=int, default=10000,
                        help='nombre de tests effectués sur des arbres aléatoirement générés')
    args = parser.parse_args()

    # importation de minimax
    cmd = f'from ..minimax.{args.version} import minimax_ab'
    print(cmd)
    exec(cmd)
