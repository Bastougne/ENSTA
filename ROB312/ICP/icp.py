...
def icp(model, data, maxIter, thres):
    ...
    minResolution = 0.05 # a threshold around 0.05 seems to be a good compromise
    prevPt = dat[:, 0]
    dat_filtered = []
    ...
    for iter in range(maxIter):
        ...
        best_matching = 0.8 # A reasonable compromise is to keep between 80% and 90% of the matchings
        sorted_dist = np . sort ( distance )
        valid = distance <= sorted_dist[int(best_matching * (len( sorted_dist) - 1))] # only best matching
